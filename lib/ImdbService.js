...
class ImdbService {
    ...
    createSeries(req, res) {
        ...
        this.imdb.findOne({_id: series.id}, function (err, item) {
            ...
            var data = {};
            data.success = true;
            self.imdb.insertOne(series.convertToMongoDbItem(), function (err, item) {
                if (err) {
                    return ImdbService.makeError(res, 500, "Can't insert to database");
                }
    ...
    getInfo(req, res) {
        ...
        this.imdb.findOne({_id: "0000000"}, function (err, item) {
            ...
            if (!item) {
                data.data.lastModified = Math.floor(new Date().getTime()) - 10;
                data.data.lastScraped = Math.floor(new Date().getTime()) - 10;
                data.data.nextScrape = Math.floor(new Date().getTime()) - 10;
                data._id = "0000000";
